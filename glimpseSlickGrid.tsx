...
// Check Value
const CheckedValue = ({ checked }: { checked: boolean }): HTMLElement => {
...
const PathNavigationElement = ({ path, tag, onClick, row }: PathNavigationElementProps): HTMLElement => {
  ...
  link.href = '#'; // Assuming you're using a JavaScript function to handle the click
  link.textContent = path;
  link.title = `Navigate to diff page for ${path} against ${tag}`;
  link.onclick = (e) => {
    e.preventDefault();
    onClick(row);
  };
...
export const PathsGrid: React.FC<PathsGridProps> = (props: PathsGridProps) => {
  ...
  const columns: Column<any>[] = columnOrder.map((col) => ({
    id: col.key as string,
    name: col.header === 'IML' ? '<a href="' + IML_DOC_LINK + '" title="If this is checked, files are running with &quot;ImportMode: Latest&quot; mode, click on the link to navigate to documentation">IML</a>' : col.header,
    field: col.key as string, // Ensure this is a string
    formatter: (row, cell, value, columnDef, dataContext) => {
      let cellContent;
      if (col.key === 'iml') {
        cellContent = CheckedValue({ checked: value as boolean });
      } else if (col.key === 'path') {
      ...
      if (col.key === 'path' && imlFiles?.includes(dataContext.path as string)) {
        cellElement.classList.add('iml-cell');
      }
    ...
    cssClass: (col.key === 'path' && imlFiles?.includes(dataContext.path as string)) ? 'iml-cell' : ''
  }));
