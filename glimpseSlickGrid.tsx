...
const styles = StyleSheet.create({
  gridStyle: {
    display: 'grid',
    gridTemplateColumns: 'repeat(4, 1fr)', // 4 groups per row
    gridColumnGap: "10px",
    width: "100%"
  },
  groupHeader: {
...
const ApproverListItem: React.FC<ApproverItemProp> = ({ approver, isApproverSelectable, isApproverSelected, onApproverCheckClicked }) => {
  ...
  return (
    <li className={css(styles.approverItemStyle)}>
      {isApproverSelectable && <VSCodeCheckbox checked={checked} onClick={onToggleChecked} />}
      <FormLabel style={styles.approverLabelStyle}>{`${approver.displayName} ${userToLink(approver.userName, approver.powwow)} ${approver.powwow}`}</FormLabel>
    </li>
  );
};
...
const ApproverListView: React.FC<ApproverListProp> = ({ group, render, onSelectGroup, selected, onClickGroupLink }) => {
  ...
  return (
    <div>
      <span>{onSelectGroup && <VSCodeCheckbox checked={checked} onClick={onToggleGroupChecked} />}
        <FormLabel style={styles.groupHeader} onClick={onGroupClick}>{createCopyLink(roleName, initials, tooltip)}</FormLabel>
      </span>
  ...
  return (
    <div className={css(styles.gridStyle)}>
      {groups.map((group, index) => (
        <div key={`quack_approver_${index}`} className={css(styles.treeNode)}>
          <ApproverListView
            group={group}
